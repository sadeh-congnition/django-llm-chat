...
def llm_call_list(request):
    ...
    llm_calls = (
        LLMCall.objects.select_related()
        .prefetch_related(
            Prefetch(
                "messages", queryset=Message.objects.order_by("date_created", "id")
            )
        )
        .order_by("-id")
    )
    ...
    return render(
        request, "django_llm_chat/llm_call_list.html", {"llm_calls": llm_calls}
    )
...
def llm_call_detail(request, call_id):
    ...
    return render(
        request, "django_llm_chat/llm_call_detail.html", {"llm_call": llm_call}
    )
